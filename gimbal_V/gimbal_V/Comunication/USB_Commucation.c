...
extern void Self_aim(float x,float y,float z,float *yaw,float *pitch,float *distance);
...
void cdc_vcp_data_rx (uint8_t *buf, uint32_t Len)
{
    for(uint16_t j=0;j<Len;j++)
    {
        ...
        {   
            //弹道解算，步兵火控
            Self_aim(pack_rx.target_x,pack_rx.target_y,pack_rx.target_z,&vision_sent.yaw.target_angle,&vision_sent.pitch.target_angle,&distance);
            //自瞄标志位
...
{ 
	...
	pack.detect_color =1;
	pack.target_id    =0x01;
	pack.roll         =-gimbal_r.IMU_actualangle;
	pack.pitch        =gimbal_p.IMU_actualangle;
	pack.yaw          =gimbal_y.IMU_actualangle;
	pack.crc16        =0xffff;
